...
from funcs.tbl_trade_diff import change_ratio_by_sector
...
def report_sector_change_ratio():
    latest_date = get_latest_date()
    date_report = str(conv_timestamp2date(latest_date))
    print('Generating change ratio of (close - open) report for %s...' % date_report)
    sector_delta = change_ratio_by_sector(latest_date)
