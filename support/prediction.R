...
for (int_date in int_date_list[, 1]) {
  ...
  for (pred_code in pred_code_list[, 1]) {
    ...
    tbl[, "Y"] <- c(tbl[2:nrow(tbl), grep(pred_code_open, names(tbl))], NA)
    
    tbl.train <- tbl[tbl$int_date < int_date, grep("[^(int_date)|(deal_date)]", names(tbl))]
    tbl.test <- tbl[tbl$int_date == int_date, grep("[^(int_date)|(deal_date)|Y]", names(tbl))]
    ...
    result <- data.frame(Date = as.Date.numeric(int_date), Predict = y.pred, Date.Next = as.Date.numeric(int_date.next), Actual = y.test.answer)
